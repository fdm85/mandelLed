...
static void mtrx_setLedsScaled(LedChainDesc_t *const lcd, channel_t *chan,
...
static void mtrx_setAuxLedsScaled(LedChainDesc_t *const lcd, channel_t *chan,
	...
	uint8_t j;
	for (j = 0; j < scaled; ++j) {
		led_setLedToColor(lcd, chan->bar[1].dots[((barHeigth / 2u) - 1) - j], r,
				g, b);
	}
	if (scaled < (barHeigth / 2)) {
		...
		led_setLedToColor(lcd, chan->bar[1].dots[((barHeigth / 2u) - 1) - j],
				remR, remG, remB);
	}
}
...
void mtrx_anim(mAnim_t *ctx) {
	...
	auxLeft = getLChanVal(e63Hz) + getLChanVal(e160Hz) + getLChanVal(e400Hz)
			+ getLChanVal(e1kHz) + getLChanVal(e2_5kHz) + getLChanVal(e6_25kHz)
			+ getLChanVal(e16kHz);
	auxRight = getRChanVal(e63Hz) + getRChanVal(e160Hz) + getRChanVal(e400Hz)
	...
	mtrx_setLedsScaled(ctx->lcd_ctx, &lLeft[2], getLChanVal(e400Hz), color,
			color, 2u);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lLeft[3], getLChanVal(e1kHz), 2u, color,
			2u);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lLeft[4], getLChanVal(e2_5kHz), color / 2,
			color / 2, color);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lLeft[5], getLChanVal(e6_25kHz), color,
			2u, 2u);
	...
	mtrx_setLedsScaled(ctx->lcd_ctx, &lRight[2], getRChanVal(e400Hz), color,
			color, 2u);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lRight[3], getRChanVal(e1kHz), 2u, color,
			2u);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lRight[4], getRChanVal(e2_5kHz),
			color / 2, color / 2, color);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lRight[5], getRChanVal(e6_25kHz), color,
			2u, 2u);
	mtrx_setLedsScaled(ctx->lcd_ctx, &lRight[6], getRChanVal(e16kHz), color / 2,
			color, color / 2);
}
